...
class PeotryGenerator(metaclass=Singleton):
    """
        诗词生成器(单例模式)
    """
    ...
    def __init__(self):
        ...
        y_true = self.model.input[0][:, 1:]
        # 目标mask
        y_mask = self.model.get_layer('Embedding-Token').output_mask[:, 1:]
        y_mask = tf.cast(y_mask, tf.float32)
        # 预测结果，到倒数第二个（包括）时结束
        y_pred = self.model.output[:, :-1]
        cross_entropy = tf.keras.losses.sparse_categorical_crossentropy(y_true, y_pred)
        cross_entropy = tf.reduce_sum(cross_entropy * y_mask) / tf.reduce_sum(y_mask)
        self.model.add_loss(cross_entropy)
        self.model.compile(tf.keras.optimizers.Adam(1e-5))
    ...
    def generate_random_poetry(self, s=''):
        ...
        while len(token_ids) + len(target_ids) < settings.MAX_LEN:
            ...
            _probas = self.model.predict([[_target_ids, ], [_segment_ids, ]])[0, -1, 3:]
            ...
            if target == 3:
                break
        return self.tokenizer.decode(token_ids + target_ids)
    ...
    def generate_acrostic(self, head):
        ...
        punctuations = ['，', '。']
        punctuation_ids = {self.tokenizer.token_to_id(token) for token in punctuations}
        # 缓存生成的诗的list
        poetry = []
        # 对于藏头诗中的每一个字，都生成一个短句
        for ch in head:
            ...
            poetry.append(ch)
            # 将藏头诗的字符转成token id
            token_id = self.tokenizer.token_to_id(ch)
            # 加入到列表中去
            ...
            while True:
                # 进行预测，只保留第一个样例（我们输入的样例数只有1）的、最后一个token的预测的、不包含[PAD][UNK][CLS]的概率分布
                _probas = self.model.predict([[token_ids, ], [segment_ids, ]])[0, -1, 3:]
                # 按照出现概率，对所有token倒序排列
                p_args = _probas.argsort()[::-1][:100]
                # 排列后的概率顺序
                p = _probas[p_args]
                ...
                if target > 3:
                    poetry.append(self.tokenizer.id_to_token(target))
